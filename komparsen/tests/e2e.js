...
(async () => {
  ...
  const files = fs.readdirSync(mdir).filter(f => f.endsWith('.txt') && fs.statSync(mdir + '/' + f).isFile()).sort((a, b) => fs.statSync(mdir + '/' + a).mtimeMs - fs.statSync(mdir + '/' + b).mtimeMs);
  ...
  let pfiles = fs.readdirSync(mdir).filter(f => f.endsWith('.txt') && fs.statSync(mdir + '/' + f).isFile()).sort((a, b) => fs.statSync(mdir + '/' + a).mtimeMs - fs.statSync(mdir + '/' + b).mtimeMs); let plast = pfiles[pfiles.length - 1];
  ...
  const files2 = fs.readdirSync(mdir).filter(f => f.endsWith('.txt') && fs.statSync(mdir + '/' + f).isFile()).sort((a, b) => fs.statSync(mdir + '/' + a).mtimeMs - fs.statSync(mdir + '/' + b).mtimeMs);
  ...
  assert(r.status === 200 && typeof r.body.impressum === 'string', 'GET /api/impressum -> 200 (Bug behoben, leer wenn unvollständig)');
  ...
  const files3 = fs.readdirSync(mdir).filter(f => f.endsWith('.txt') && fs.statSync(mdir + '/' + f).isFile()).sort((a, b) => fs.statSync(mdir + '/' + a).mtimeMs - fs.statSync(mdir + '/' + b).mtimeMs);
